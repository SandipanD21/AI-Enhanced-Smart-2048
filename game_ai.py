...
SPM_SCALE_PARAM = 25 # Number of simulations that are run for each possible move at each level of the tree      
SL_SCALE_PARAM = 10 # Number of moves that are considered in each simulation
SEARCH_PARAM = 200

from game_functions import initialize_game, random_move,move_down, move_left, move_right, move_up,check_for_win, add_new_tile
                          

# calculates the values of SPM and SL based on the move number. The parameters are increased 
# as the game progresses to allow for deeper searches in the later stages of the game.
...
# It takes the current game board, the number of simulations to run, and the length of each simulation 
# as inputs. It uses a loop to simulate the game for each possible first move (left, right, up, down) 
# and keeps track of the score for each move. The move with the highest score is returned
        ...
        for _ in range(searches_per_move):
            ...
            while game_valid and move_number < search_length:
                ...
                if game_valid:
                    ...
                    move_number += 1
    best_move_index = np.argmax(first_move_scores) # np.argmax() returns the index of the maximum value in the numpy array
...
def ai_play(board):
    ...
    while valid_game:
        ...
        board, valid_game = ai_move(board, number_of_simulations, search_length) # Now board contains the board that exists after the move_number th move is made
        if valid_game:
